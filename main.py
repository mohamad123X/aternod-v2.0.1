...
# إعدادات البوت الأساسية مع الصلاحيات الكاملة
...
# --- 1. دالات إدارة قاعدة البيانات البسيطة ---
def load_db():
...
async def control_aternos(action: str, username: str, password: str, account_type: str, server_ip: str):
    async with async_playwright() as p:
        # إعداد متصفح مخفي بمواصفات تخفي لمنع الحظر
        ...
        try:
            ...
            # التعامل مع نوع الحساب (إذا كان متعاون Shared Access)
            if account_type == "shared":
                # في الحسابات المشتركة ننتظر الانتقال لصفحة السيرفرات المشتركة أو نحدد السيرفر المطلوب
                await page.wait_for_timeout(3000)
            
            # انتظار قائمة السيرفرات والضغط على السيرفر المطلوب بناءً على الاسم أو الـ IP المكتوب
            await page.wait_for_selector(".server-body", timeout=10000)
            
            if server_ip and server_ip != "الأول":
                # البحث عن السيرفر بالاسم المكتوب
                ...
                if await server_element.count() > 0:
                    ...
                    await page.click(".server-body") # الضغط على الأول تلقائياً كبديل
            else:
                await page.click(".server-body") # الضغط على أول سيرفر تلقائياً
            
            # تحديد الإجراء (تشغيل أم إيقاف)
            if action == "start":
                await page.wait_for_selector("#start", timeout=10000)
                # التحقق إذا كان الزر متاحاً للتشغيل وليس أونلاين بالفعل
                status = await page.locator(".statuslabel-label").inner_text()
                if "Offline" in status:
                    await page.click("#start")
                    msg = "✅ تم إرسال أمر تشغيل السيرفر بنجاح! السيرفر الآن قيد التحضير."
                else:
                ...
                if "Online" in status or "Starting" in status:
                    ...
                    msg = "🛑 تم إرسال أمر إطفاء السيرفر بنجاح! جاري الإغلاق."
                else:
                    msg = f"⚠️ لا يمكن إغلاق السيرفر لأنه بالحالة: {status}"
            ...
            await browser.close()
            return f"❌ فشل الاتصال بأترنوس. قد يكون بسبب نظام حماية الموقع أو خطأ بالبيانات. تفاصيل: {e}"

# --- 3. أزرار لوحة التحكم المستمرة (Persistent Control Panel) ---
class AternosControlView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None) # التايم أوت None لكي لا تتوقف الأزرار عن العمل أبداً

    async def check_permissions(self, interaction: discord.Interaction) -> bool:
        """التحقق مما إذا كان المستخدم يملك رتبة مسموحة أو رتبة مسؤول"""
        db = load_db()
        guild_id = str(interaction.guild_id)
        
        if guild_id not in db:
            await interaction.response.send_message("❌ لم يتم العثور على إعدادات لهذا السيرفر.", ephemeral=True)
            return False
            
        config = db[guild_id]
        # إذا كان مسؤولاً، يتخطى الفحص تلقائياً
        if interaction.user.guild_permissions.administrator:
            return True
            
        # فحص رتب المستخدم المطابقة للرتب المسموحة
        ...
        await interaction.response.send_message("⛔ عذراً، لا تمتلك رتبة مسموح لها بالتحكم في السيرفر.", ephemeral=True)
        return False
    ...
    async def start_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("⏳ جاري التواصل مع أترنوس لتشغيل السيرفر... يرجى الانتظار.", ephemeral=True)
        db = load_db()
        config = db[str(interaction.guild_id)]
        
        # استدعاء دالة الأتمتة
    ...
    async def stop_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("⏳ جاري التواصل مع أترنوس لإيقاف السيرفر... يرجى الانتظار.", ephemeral=True)
        db = load_db()
        config = db[str(interaction.guild_id)]
...
class AternosSetupModal(discord.ui.Modal, title="⚙️ إعداد وتثبيت بيانات أترنوس"):
    ...
    allowed_roles = discord.ui.TextInput(label="الرتب المسموح لها بالتحكم (افصل بفاصلة)", placeholder="مثال: Admin, Member, Mod")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            # 1. إنشاء القناة النصية الجديدة للتحكم
            new_channel = await guild.create_text_channel(name=self.channel_name.value)
            
            # 2. حفظ البيانات داخل قاعدة البيانات المحلية لكل سيرفر ديسكورد
            db = load_db()
            db[str(guild.id)] = {
            ...
            # 3. إرسال لوحة التحكم والأزرار داخل القناة الجديدة
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "⚠️ **ملاحظة:** الرتب المسموح لها بالضغط على الأزرار هي فقط المحددة أثناء الإعداد بالإضافة للمسؤولين (Administrators)."
                ),
            ...
            # تأكيد نجاح العملية للمنشئ
            await interaction.followup.send(content=f"✅ تم التثبيت بنجاح! تم إنشاء القناة هنا: {new_channel.mention}", ephemeral=True)
            
        except Exception as e:
            await interaction.followup.send(content=f"❌ حدث خطأ أثناء إعداد القناة: {e}", ephemeral=True)
...
class AccountTypeSelect(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(placeholder="اختر نوع ملكية حساب أترنوس...", min_values=1, max_values=1, options=options)

    async def callback(self, interaction: discord.Interaction):
        # عند الاختيار، نقوم بفتح النافذة المنبثقة مباشرة وتمرير خيار المستخدم لها
...
# --- 6. الأوامر المائلة وأحداث تشغيل البوت ---
@bot.event
async def on_ready():
    print(f"🔥 تم تشغيل بوت {bot.user} بنجاح وهو جاهز للعمل!")
    
    # ربط وتثبيت الأزرار الثابتة لكي لا تموت عند إعادة تشغيل البوت
    bot.add_view(AternosControlView())
    
    try:
        synced = await bot.tree.sync()
        print(f"🔄 تم مزامنة {len(synced)} أوامر مائلة بنجاح.")
    except Exception as e:
        print(f"❌ فشل في مزامنة الأوامر: {e}")

# الأمر المائل الأساسي للتثبيت والتجهيز
@bot.tree.command(name="setup_aternos", description="إعداد وتثبيت سيرفر أترنوس بإنشاء قناة وأزرار تحكم")
@app_commands.checks.has_permissions(administrator=True) # مسموح فقط للمسؤولين باستخدامه لتجنب التخريب
async def setup_aternos(interaction: discord.Interaction):
    view = AccountTypeView()
    await interaction.response.send_message("🛠️ أولاً، يرجى تحديد نوع ملكية الحساب الخاص بك من القائمة أدناه للبدء:", view=view, ephemeral=True)

@setup_aternos.error
async def setup_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.errors.MissingPermissions):
        await interaction.response.send_message("⛔ عذراً، هذا الأمر مخصص لمسؤولي السيرفر (Administrators) فقط.", ephemeral=True)

# تشغيل البوت بالتوكن الخاص به
